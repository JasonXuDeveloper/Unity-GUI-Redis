    ...
    public class RedisWindow : EditorWindow, IHasCustomMenu
    {
        ...
        void TestConnection()
        {
            ...
            if (IsConnecting)
            {
                if (Language == Language.中文)
                {
                    Core.LogError("连接正在进行");
                }
                else
                {
                    Core.LogError("It is currently connecting the server");
                }
                return;
            }
            IsConnecting = true;
        ...
        void Details()
        {
            if (Language == Language.中文)
            {
                Core.LogWarning("关于插件");
                Core.Log("插件名称: GUI Redis");
                Core.Log("作者: 傑");
                Core.Log("版本: v1.0");
                Core.LogWarning("插件功能");
                Core.Log("连接Redis数据库");
                Core.Log("查询数据");
                Core.Log("增加数据");
                Core.Log("修改数据");
                Core.Log("删除数据");
                Core.LogError("使用须知");
                Core.LogWarning("目前版本仅支持最基础的key-value查询");
                Core.LogWarning("Hash, List, Set等暂不支持");
            }
            else
            {
                Core.LogWarning("About asset");
                Core.Log("Asset name: GUI Redis");
                Core.Log("Author: JasonXuDeveloper");
                Core.Log("Version: v1.0");
                Core.LogWarning("Features");
                Core.Log("Connect Redis Database");
                Core.Log("Get Data");
                Core.Log("Add Data");
                Core.Log("Edit Data");
                Core.Log("Delete Data");
                Core.LogError("Notes");
                Core.LogWarning("Current version only supports operating key-value data");
                Core.LogWarning("Hash, List, Set are currently unavaliable");
            }
        }
